// index.cjs
// Mutual Fund Telegram Bot with SIP Support + Register + Lead Capture
// Features:
...
// - Auto SIP execution endpoint: GET /cron/run-sips?token=YOUR_CRON_SECRET
...
function addMonthsToYmd(ymd, months) {
  ...
  const lastDay = temp.getUTCDate();
  const safeDay = Math.min(day, lastDay);
  return new Date(Date.UTC(year, month + months, safeDay)).toISOString().slice(0, 10);
}
...
function round4(num) { return Math.round((Number(num) + Number.EPSILON) * 10000) / 10000; }
function parseAmount(value) {
  const cleaned = String(value || "").replace(/[,₹\s]/g, "");
  const amt = Number(cleaned);
  return Number.isFinite(amt) && amt > 0 ? amt : null;
}
function commandBody(text) { return String(text || "").split(" ").slice(1).join(" ").trim(); }
...
function parseSipCommand(text) {
  ...
  if (!fundName || !amount || !["monthly"].includes(frequency)) return null;
...
function scoreMatch(inputNorm, schemeNorm) {
  ...
  let common = 0;
  for (const w of inputWords) if (schemeWords.has(w)) common++;
  return common;
}
...
async function sendTelegramMessage(chatId, text, extra = {}) {
  const payload = { chat_id: chatId, text, parse_mode: "HTML", disable_web_page_preview: true, ...extra };
  const resp = await fetch(`${TELEGRAM_API}/sendMessage`, {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload)
  });
  const data = await resp.json().catch(() => ({}));
  if (!resp.ok || !data.ok) console.error("Telegram sendMessage error:", data);
  return data;
}
async function setWebhookFromRailwayUrl(baseUrl) {
  ...
  const resp = await fetch(`${TELEGRAM_API}/setWebhook`, {
    ...
    body: JSON.stringify({ url: `${baseUrl}${WEBHOOK_PATH}` }),
  });
  console.log("setWebhook response:", await resp.json().catch(() => ({})));
}
...
async function refreshNavCache(force = false) {
  ...
  const resp = await fetch(AMFI_NAV_URL, { headers: { "User-Agent": "Mozilla/5.0 MF Telegram Bot", "Accept": "text/plain,text/html,*/*" } });
  if (!resp.ok) throw new Error(`Failed to fetch NAV feed: ${resp.status}`);
...
async function resolveSchemeByName(fundName) {
  ...
  for (const scheme of navCache.all) {
    ...
    if (score > bestScore) { bestScore = score; best = scheme; }
  }
  return best && bestScore >= 2 ? best : null;
}
async function getSchemeByCodeOrName(code, name) {
  await refreshNavCache(false);
  if (code && navCache.byCode.has(String(code))) return navCache.byCode.get(String(code));
  return resolveSchemeByName(name);
}

async function getUserHoldings(chatId) {
  const { data, error } = await supabase.from(HOLDINGS_TABLE).select("*").eq(USER_COL, String(chatId));
...
async function upsertHoldingById(existingRow, payload) {
  if (existingRow && existingRow.id) {
    const { data, error } = await supabase.from(HOLDINGS_TABLE).update(payload).eq("id", existingRow.id).select().single();
    if (error) throw error;
    return data;
  }
  const { data, error } = await supabase.from(HOLDINGS_TABLE).insert(payload).select().single();
  if (error) throw error;
  return data;
}
async function deleteHoldingById(id) {
  const { error } = await supabase.from(HOLDINGS_TABLE).delete().eq("id", id);
  if (error) throw error;
}
...
async function getDistinctChatIds() {
  const { data, error } = await supabase.from(HOLDINGS_TABLE).select(USER_COL).neq(USER_COL, null);
  if (error) throw error;
...
async function markLead(chatId) {
  try {
    const { error } = await supabase.from(USERS_TABLE).upsert(
      { chat_id: String(chatId), is_lead: true, updated_at: nowIso() },
      { onConflict: "chat_id" }
    );
    if (error) throw error;
    return true;
...
async function addFund(chatId, fundName, amount) {
  const scheme = await resolveSchemeByName(fundName);
  if (!scheme) return { ok: false, message: `❌ Fund not found for: <b>${escapeHtml(fundName)}</b>\n\nTry:\n<code>/add hdfc flexi cap | 5000</code>` };
...
async function buildSummary(chatId, compact = false) {
  const holdings = await getUserHoldings(chatId);
  if (!holdings.length) return compact ? `📭 No holdings found.` : `📭 <b>No holdings found.</b>\n\nUse:\n<code>/add hdfc flexi cap | 5000</code>`;
  await refreshNavCache(false);
  let totalInvested = 0, totalCurrent = 0, best = null, worst = null;
  for (const h of holdings) {
    const scheme = await getSchemeByCodeOrName(h[SCHEME_CODE_COL], h[FUND_NAME_COL]);
    const fundName = scheme?.name || h[FUND_NAME_COL] || "Unknown Fund";
    const nav = Number(scheme?.nav || h[LAST_NAV_COL] || 0);
...
async function handleTextMessage(chatId, text) {
  ...
  try {
    ...
    if (lower.startsWith("/add ")) {
      ...
      if (!parsed) return await sendTelegramMessage(chatId, `❌ Invalid format.\n\nUse:\n<code>/add hdfc flexi cap | 5000</code>`);
    ...
    if (lower === "/portfolio") return await sendTelegramMessage(chatId, await buildPortfolioText(chatId));
    if (lower === "/summary") return await sendTelegramMessage(chatId, await buildSummary(chatId, false));
    if (lower.startsWith("/sip ")) {
      const parsed = parseSipCommand(normalized);
      if (!parsed) return await sendTelegramMessage(chatId, `❌ Invalid SIP format.\n\nUse:\n<code>/sip hdfc flexi cap | 5000 | monthly</code>`);
      const result = await createOrUpdateSip(chatId, parsed.fundName, parsed.amount, parsed.frequency);
      return await sendTelegramMessage(chatId, result.message);
    }
    if (lower === "/sips") return await sendTelegramMessage(chatId, await buildSipsText(chatId));
    ...
    await sendTelegramMessage(chatId, `⚠️ Something went wrong.\n\n<code>${escapeHtml(err.message || "Unknown error")}</code>`);
  }
}
...
app.get("/refresh-nav", async (_req, res) => {
  try {
    ...
    res.json({ ok: true, count: navCache.all.length, lastFetch: new Date(navCache.lastFetchMs).toISOString() });
  } catch (err) {
...
app.get("/cron/summary", async (req, res) => {
  try {
    if (String(req.query.token || "") !== CRON_SECRET) return res.status(401).json({ ok: false, error: "Unauthorized" });
    await refreshNavCache(true);
    ...
    for (const chatId of chatIds) {
      try {
        ...
        if (!msg || msg.includes("No holdings found")) { skipped++; continue; }
...
app.get("/cron/run-sips", async (req, res) => {
  try {
    ...
    const { data: dueRows, error } = await supabase.from(SIPS_TABLE).select("*").eq("is_active", true).lte("next_due_date", today);
    if (error) throw error;
    let processed = 0, failed = 0;
    for (const row of dueRows || []) {
      try {
        const out = await addFund(row[USER_COL], row[FUND_NAME_COL], Number(row.amount || 0));
        if (!out.ok) throw new Error(out.message);
        const nextDue = String(row.frequency || "monthly").toLowerCase() === "monthly" ? addMonthsToYmd(row.next_due_date || today, 1) : today;
        const { error: updateError } = await supabase.from(SIPS_TABLE).update({ last_run_date: today, next_due_date: nextDue, updated_at: nowIso() }).eq("id", row.id);
        if (updateError) throw updateError;
        await sendTelegramMessage(row[USER_COL], `🔁 <b>SIP executed</b>\n\n📌 Fund: <b>${escapeHtml(row[FUND_NAME_COL])}</b>\n💰 Amount: <b>${formatINR(row.amount)}</b>\n📅 Next Due: <b>${escapeHtml(formatDisplayDate(nextDue))}</b>`);
        processed++;
      } catch (e) {
    ...
    res.json({ ok: true, today, totalDue: (dueRows || []).length, processed, failed });
  } catch (err) {
...
app.get("/test-send", async (req, res) => {
  try {
    if (!req.query.chat_id) return res.status(400).json({ ok: false, error: "chat_id required" });
    res.json({ ok: true, result: await sendTelegramMessage(req.query.chat_id, escapeHtml(String(req.query.msg || "Test message from bot"))) });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
  }
});
...
app.listen(PORT, async () => {
  ...
  try { await refreshNavCache(true); } catch (e) { console.warn("Initial NAV cache load failed:", e.message); }
  const railwayPublicDomain = process.env.RAILWAY_STATIC_URL || process.env.RAILWAY_PUBLIC_DOMAIN || process.env.APP_URL || null;
  if (railwayPublicDomain) {
    const baseUrl = railwayPublicDomain.startsWith("http") ? railwayPublicDomain : `https://${railwayPublicDomain}`;
